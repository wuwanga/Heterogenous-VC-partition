...
{ 
   ...
   if (fixed_lat_icnt) {
	 ...
	 int node = -1;
         ...
         traffic[nc]->_GeneratePacket( input, n_flits, 0 /*class*/, traffic[nc]->_time, data, output, node);
      }
                        ...
                        unsigned int n_cpu)
{
   ...
   for ( int i = 0; i < 36; ++i ) {
      for ( int j = 0; j < 4; ++j ) {
         _kalman_gain[i][j] = 0.0;
         _err_estimate[i][j] = 1.0;
         _current_estimate[i][j] = 2.0;
         _last_estimate[i][j] = 0.0;
      }
   }
   _err_measure = 2;
