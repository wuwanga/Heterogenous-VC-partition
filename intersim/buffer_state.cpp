...
int BufferState::FindAvailable( Flit *f, int node )
{
   int available_vc = -1;
   int vc;
   ...
   //if (f->nodes == 1 || f->nodes == 3) {
   int n_vcs = 2;

   //_last_avail = RandomInt( _vcs - 1 );
   _last_avail = RandomInt( n_vcs - 1 );

   for ( int v = 0; v < n_vcs; ++v ) {
      if (f->nodes/2) {
         vc = ( v + _last_avail + 1 ) % n_vcs; // Round-robin
      } else {
         vc = ( v + _last_avail + 1 ) % n_vcs + n_vcs; // Round-robin
     }
   //for ( int v = 0; v < _vcs; ++v ) {
   //   vc = ( v + _last_avail + 1 ) % _vcs;
